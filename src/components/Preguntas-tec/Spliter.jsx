...
const Spliter = ({ children, initialSize = 80 }) => {
    ...
    return (
        <div  className="rounded-lg w-full base:w-1/2 base:gap-1">
            <div ref={containerRef} className='h-full w-full flex flex-col space-y-2.5 base:space-y-1'>
                <div className={`w-full `} style={!isSmallScreen ? { height: `${size}%` } : {}}>
                    {firstChild}
                </div>
                <div onMouseDown={handleMouseDown} className='hidden base:block cursor-ns-resize  w-full hover:bg-blue-500 rounded-full'>
                    <div className="w-8 mx-auto h-1 bg-gray-500/30 rounded-full"/>
                </div>
